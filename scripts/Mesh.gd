...
func set_start(val:bool)->void:
	if Engine.is_editor_hint():
		create_dungeon()
var dun_cell_scene : PackedScene = preload("res://scenes/main/cell2.tscn")
...
func create_dungeon():
	...
	for cell in grid_map.get_used_cells():
		...
		&& cell_index >=0:
			...
			dun_cell.position = Vector3(cell) + Vector3(cell) + Vector3(0.5,0,0.5)
			dun_cell.transform.scaled_local(Vector3(2.0,2.0,2.0))
			add_child(dun_cell)
